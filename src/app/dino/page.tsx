...
const DinoPage: React.FC = () => {
  ...
  const indicatorHitTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  const checkCollision = useCallback(() => {
    ...
    const obstacles = [cactus1Ref.current, cactus2Ref.current, pterodactylRef.current];
  ...
  return (
    ...
    >
      {isGodMode && 
        <div className={`${styles.godModeIndicator} ${isIndicatorHit ? styles.indicatorHit : ''}`}>
      ...
      <>
        <div ref={cactus1Ref} className={`${styles.obstacle} ${styles.cactus1}`}>🌵</div>
        <div ref={cactus2Ref} className={`${styles.obstacle} ${styles.cactus2}`}>🌵</div>
        <div ref={pterodactylRef} className={`${styles.obstacle} ${styles.pterodactyl}`}>🦅</div>
      </>

      <div className={styles.ground}></div>
        
      <p className={styles.instructions}>
        Press Space/Up/W to Jump.
      </p>
    </motion.div>
  );
};
