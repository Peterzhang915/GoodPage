...
export default async function Home() {
  ...
  try {
    ...
    if (res.ok) { // 请求成功 (status 2xx)
      ...
      if (responsePayload && responsePayload.success === true && responsePayload.data) {
         ...
         if (newsResult && typeof newsResult.title === 'string' && Array.isArray(newsResult.news) && newsResult.news.every((item: unknown) => typeof item === 'string')) {
          ...
          const errorMessage = responsePayload?.error?.message || 'Invalid response format from server.';
          fetchError = `Failed to load news: ${errorMessage}`;
      }
    } else { // 请求失败 (status 非 2xx)
      let errorMessage = `Server error (${res.status})`;
      try {
          // 尝试解析错误响应体 (可能包含标准错误结构)
          const errorPayload = await res.json();
          if (errorPayload && errorPayload.success === false && errorPayload.error && errorPayload.error.message) {
  ...
  } catch (error: any) { // 请求过程中发生网络或其他错误
    isLoading = false; // 请求完成，设置 loading 为 false
