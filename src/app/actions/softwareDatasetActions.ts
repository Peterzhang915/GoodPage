...
const SoftwareDatasetFormSchema = z.object({
  ...
  repositoryUrl: z
    .string()
    .url("Invalid URL format for repository link.")
    .optional()
    .or(z.literal(""))
    .nullable(),
  projectUrl: z
    .string()
    .url("Invalid URL format for project link.")
    .optional()
    .or(z.literal(""))
    .nullable(),
 ...
 */
export async function addSoftwareDatasetRecord(
  memberId: string,
  data: SoftwareDatasetFormData
): Promise<{
  success: boolean;
  error?: string;
  softwareDataset?: Prisma.SoftwareDatasetGetPayload<{}>;
}> {
  try {
    ...
    return {
      success: false,
      error: error.message || "Failed to add software/dataset record.",
    };
  }
}
 ...
 */
export async function updateSoftwareDatasetRecord(
  recordId: number,
  data: SoftwareDatasetFormData
): Promise<{
  success: boolean;
  ...
  try {
    ...
    const existingRecord = await prisma.softwareDataset.findUnique({
      where: { id: recordId },
    });
    if (!existingRecord) {
    ...
    return {
      success: false,
      error: error.message || "Failed to update software/dataset record.",
    };
  }
}
 ...
 */
export async function deleteSoftwareDatasetRecord(
  recordId: number
  ...
  try {
    ...
    return {
      success: false,
      error: error.message || "Failed to delete software/dataset record.",
    };
  }
}
