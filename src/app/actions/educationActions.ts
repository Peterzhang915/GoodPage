...
export async function addEducationRecord(
  ...
  if (!data.school || !data.degree || !data.start_year) {
    return {
      success: false,
      error: "School, Degree, and Start Year are required.",
    };
  }
  ...
  // }
  console.warn(
    `Action Warning: Permission check disabled in addEducationRecord for member ${member_id}`
  );
  ...
  try {
    const newEducation = await prisma.education.create({
      data: {
        ...
        start_year: data.start_year
          ? parseInt(data.start_year.toString(), 10)
          : null,
    ...
    console.log(
      `Action: Successfully added education record ${newEducation.id} for member ${member_id}`
    );
    ...
    console.error(
      `Action: Error adding education for member ${member_id}:`,
      error
    );
    ...
    return {
      success: false,
      error: "An unknown error occurred while adding education.",
    };
  }
}
...
export async function updateEducationRecord(
  ...
  // }
  console.warn(
    `Action Warning: Permission check disabled in updateEducationRecord for education ${educationId}`
  );
  ...
  try {
      ...
      data: {
        ...
        end_year:
          data.end_year !== undefined
            ? data.end_year
              ? parseInt(data.end_year.toString(), 10)
              : null
            : undefined,
      },
    });
    ...
    return {
      success: false,
      error: "An unknown error occurred while updating education.",
    };
  }
}
...
export async function deleteEducationRecord(
  ...
  // }
  console.warn(
    `Action Warning: Permission check disabled in deleteEducationRecord for education ${educationId}`
  );

  try {
    // Store member_id before deleting for revalidation path
    const educationToDelete = await prisma.education.findUnique({
      where: { id: educationId },
    ...
    if (member_id) {
      ...
      revalidatePath(`/members/${member_id}`); // Use correct field name
      console.log(
        `Action: Revalidated paths after deleting education for member ${member_id}`
      );
    }
    ...
    return {
      success: false,
      error: "An unknown error occurred while deleting education.",
    };
  }
}
