...
const AcademicServiceFormSchema = z
  .object({
    ...
    startYear: z.coerce
      ...
      .optional()
      .nullable(), // Coerce string/null to number
    endYear: z.coerce
      .number()
      .int()
  ...
  .refine(
    (data) =>
      !data.startYear || !data.endYear || data.endYear >= data.startYear,
    {
 ...
 */
export async function addAcademicServiceRecord(
  memberId: string,
  data: AcademicServiceFormData
): Promise<{
  success: boolean;
  error?: string;
  academicService?: Prisma.AcademicServiceGetPayload<{}>;
}> {
  try {
    ...
    return {
      success: false,
      error: error.message || "Failed to add academic service record.",
    };
  }
}
 ...
 */
export async function updateAcademicServiceRecord(
  serviceId: number,
  data: AcademicServiceFormData
): Promise<{
  success: boolean;
  ...
  try {
    ...
    return {
      success: false,
      error: error.message || "Failed to update academic service record.",
    };
  }
}
 ...
 */
export async function deleteAcademicServiceRecord(
  serviceId: number
  ...
  try {
    ...
    return {
      success: false,
      error: error.message || "Failed to delete academic service record.",
    };
  }
}
