...
export async function addTeachingRecord(
  ...
  console.warn(
    `Action Warning: Permission check disabled in addTeachingRecord for member ${member_id}`
  );
  ...
  try {
    ...
    console.log(
      `Action: Successfully added teaching record ${newTeaching.id} for member ${member_id}`
    );
    ...
    console.error(
      `Action: Error adding teaching record for member ${member_id}:`,
      error
    );
    ...
    return {
      success: false,
      error: "An unknown error occurred while adding teaching record.",
    };
  }
}
...
export async function updateTeachingRecord(
  ...
  // Optional: Validate URL format if provided
  if (
    data.description_url !== undefined &&
    data.description_url !== null &&
    data.description_url !== "" &&
    !/^https?:\/\/.+/.test(data.description_url)
  ) {
    return { success: false, error: "Invalid Description URL format." };
  }
  ...
  // --- TODO: Permission Check ---
  console.warn(
    `Action Warning: Permission check disabled in updateTeachingRecord for teaching record ${teachingId}`
  );
  ...
  try {
    const updatedTeaching = await prisma.teaching.update({
      ...
      data: {
        ...
        year:
          data.year !== undefined
            ? data.year
              ? parseInt(data.year.toString(), 10)
              : null
            : undefined,
    ...
    console.error(
      `Action: Error updating teaching record ${teachingId}:`,
      error
    );
    if (error instanceof Error) {
      return { success: false, error: `Database error: ${error.message}` };
    ...
    return {
      success: false,
      error: "An unknown error occurred while updating teaching record.",
    };
  }
}
...
export async function deleteTeachingRecord(
  ...
  // --- TODO: Permission Check ---
  console.warn(
    `Action Warning: Permission check disabled in deleteTeachingRecord for teaching record ${teachingId}`
  );

  try {
    // Store member_id before deleting for revalidation path
    const teachingToDelete = await prisma.teaching.findUnique({
      where: { id: teachingId },
    ...
    if (member_id) {
      ...
      console.log(
        `Action: Revalidated paths after deleting teaching record for member ${member_id}`
      );
    }
    ...
    console.error(
      `Action: Error deleting teaching record ${teachingId}:`,
      error
    );
    if (error instanceof Error) {
      return { success: false, error: `Database error: ${error.message}` };
    ...
    return {
      success: false,
      error: "An unknown error occurred while deleting teaching record.",
    };
  }
}
