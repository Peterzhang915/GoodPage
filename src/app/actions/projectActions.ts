...
const ProjectFormSchema = z
  ...
  .refine(
    (data) =>
      !data.end_year || !data.start_year || data.end_year >= data.start_year,
    {
...
export async function addProjectRecord(
...
): Promise<{ success: boolean; error?: string; project?: AddProjectResult }> {
  ...
  if (!validationResult.success) {
    ...
    return {
      success: false,
      error: JSON.stringify(validationResult.error.flatten().fieldErrors),
    };
  }
  ...
  try {
    console.log(
      `Action: Adding project for member ${memberId} with data:`,
      data
    );
...
export async function updateProjectRecord(
  ...
  if (!validationResult.success) {
    console.error(
      "Update Project Validation Error:",
      validationResult.error.flatten().fieldErrors
    );
    return {
      success: false,
...
export async function deleteProjectRecord(
  ...
  try {
    console.log(
      `Action: Deleting project link for project ${projectId} and member ${memberId}`
    );
    // Use the composite key in the where clause
    await prisma.projectMember.delete({
      where: {
        project_id_member_id: {
          // Use the Prisma-generated composite key name
    ...
    console.error(
      `Error deleting project link for project ${projectId}, member ${memberId}:`,
      error
    );
