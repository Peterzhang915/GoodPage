...
const PresentationFormSchema = z.object({
    ...
    conferenceUrl: z.string().url("Invalid URL format for conference link.").optional().or(z.literal('')).nullable(), // Optional URL
    location: z.string().optional().nullable(),
    year: z.coerce.number().int().positive("Year must be a positive integer.").optional().nullable(),
    url: z.string().url("Invalid URL format for slides/video link.").optional().or(z.literal('')).nullable(), // Optional URL for slides/video
    isInvited: z.boolean(),
 ...
 */
export async function addPresentationRecord(memberId: string, data: PresentationFormData): Promise<{ success: boolean; error?: string; presentation?: Prisma.PresentationGetPayload<{}> }> {
    try {
        ...
        return { success: false, error: error.message || "Failed to add presentation record." };
    }
}
 ...
 */
export async function updatePresentationRecord(presentationId: number, data: PresentationFormData): Promise<{ success: boolean; error?: string; presentation?: Prisma.PresentationGetPayload<{}> }> {
    try {
        // await ensureAuthenticated();
        const validatedData = PresentationFormSchema.parse(data);

        const existingPresentation = await prisma.presentation.findUnique({ where: { id: presentationId } });
        ...
        return { success: false, error: error.message || "Failed to update presentation record." };
    }
}
 ...
 */
export async function deletePresentationRecord(presentationId: number): Promise<{ success: boolean; error?: string }> {
    try {
        // await ensureAuthenticated();

        const existingPresentation = await prisma.presentation.findUnique({ where: { id: presentationId }, select: { member_id: true } });
        if (!existingPresentation) {
            return { success: false, error: "Presentation record not found." };
        }
        ...
        return { success: false, error: error.message || "Failed to delete presentation record." };
    }
} 