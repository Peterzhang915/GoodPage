...
export async function POST(request: NextRequest, { params }: Params) {
    ...
    if (!memberId) {
        return NextResponse.json({ success: false, error: { message: "Member ID is required" } }, { status: 400 });
    }
    ...
    try {
        ...
        if (!data.degree || !data.school) {
             return NextResponse.json({ success: false, error: { message: "Degree and School are required" } }, { status: 400 });
        }
        ...
        console.error(`[API] Failed to create education for member ${memberId}:`, error);
