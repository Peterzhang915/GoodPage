...
export async function GET(request: NextRequest, { params }: Params) {
    ...
    if (isNaN(educationId)) {
        return NextResponse.json({ success: false, error: { message: "Invalid Education ID" } }, { status: 400 });
    }

    try {
        const education = await getEducationRecordById(educationId);
        if (!education) {
            return NextResponse.json({ success: false, error: { message: "Education record not found" } }, { status: 404 });
        }
