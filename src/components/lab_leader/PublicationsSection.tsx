...
const PublicationsSection: React.FC<PublicationsSectionProps> = ({ publications, fetchError }) => {
  return (
    <section id="publications">
      ...
      {fetchError && (<p className={`text-red-600 dark:text-red-400 ${themeColors.backgroundWhite ?? 'bg-red-50'} p-3 rounded-md mb-4`}>Error: {fetchError}</p>)}
      {!fetchError && (
        <div className="list-none space-y-3 md:space-y-4">
          {publications.length > 0 ? (
            ...
            <p className={`${themeColors.textColorSecondary ?? 'text-gray-500'}`}>No selected publications available at the moment.</p>
          )}
        </div>
      )}
    </section>
