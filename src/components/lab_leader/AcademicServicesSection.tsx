...
const AcademicServicesSection: React.FC<AcademicServicesSectionProps> = ({
  ...
  // Sorting (optional)
  const sortedFeatured = [...featuredServices].sort((a, b) => a.display_order - b.display_order);
  const sortedDetailed = [...detailedServices].sort((a, b) => a.display_order - b.display_order);
      ...
      {!fetchError && hasFeatured && (
        <ul className="list-none pl-0 space-y-2 md:space-y-3 mb-4">
          {sortedFeatured.map((service) => (
            <li key={service.id} className={`flex items-start ${themeColors.textColorSecondary ?? ''} text-sm md:text-base`}>
               <ClipboardList size={16} className={`mr-2 mt-1 flex-shrink-0 ${themeColors.textColorTertiary ?? 'text-gray-500'}`} />
               <div>
                  {service.content}
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-3"> {/* Two-column grid */}
              {sortedDetailed.map((service) => (
                 <div key={service.id} className={`flex items-start ${themeColors.textColorSecondary ?? ''} text-sm`}> {/* Use div for grid item */}
                   <ClipboardList size={16} className={`mr-2 mt-1 flex-shrink-0 ${themeColors.textColorTertiary ?? 'text-gray-400'}`} />
                   <div>
                     {service.content}
                     {/* Add link rendering if AcademicService schema includes link_url */}
                     {/* {service.link_url && ... } */}
