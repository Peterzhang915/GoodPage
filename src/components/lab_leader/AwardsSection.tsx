...
const AwardsSection: React.FC<AwardsSectionProps> = ({
  ...
  const sortedFeatured = [...featuredAwards].sort((a, b) => a.display_order - b.display_order);
  const sortedDetailed = [...detailedAwards].sort((a, b) => a.display_order - b.display_order);
  ...
  return (
    <section id="awards" className="mb-8">
      ...
      {fetchError && (
        <p className={`text-red-600 dark:text-red-400 ${themeColors.backgroundWhite ?? 'bg-red-50'} p-3 rounded-md mb-4`}>
      ...
      {!fetchError && hasFeatured && (
        <ul className="list-none pl-0 space-y-2 md:space-y-3 mb-4">
          {sortedFeatured.map((award) => (
            <li key={award.id} className={`flex items-start ${themeColors.textColorSecondary ?? ''} text-sm md:text-base`}>
              <Trophy size={16} className={`mr-2 mt-1 flex-shrink-0 ${themeColors.ccfAText ?? 'text-yellow-600'}`} />
              <div>
                <span>{award.content}</span>
                {award.year && <span className="text-xs md:text-sm ml-1">({award.year})</span>}
                {award.link_url && (
                   <a
                    href={award.link_url}
                    target="_blank"
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-3">
              {sortedDetailed.map((award) => (
                 <div key={award.id} className={`flex items-start ${themeColors.textColorSecondary ?? ''} text-sm`}>
                   <Trophy size={16} className={`mr-2 mt-1 flex-shrink-0 ${themeColors.textColorTertiary ?? 'text-gray-400'}`} />
                   <div>
                    <span>{award.content}</span>
                    {award.year && <span className="text-xs md:text-sm ml-1">({award.year})</span>}
                    {award.link_url && (
