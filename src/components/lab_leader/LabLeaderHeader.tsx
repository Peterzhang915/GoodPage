...
const LabLeaderHeader: React.FC<LabLeaderHeaderProps> = ({ leaderData }) => {
  ...
  return (
    <div className={`${themeColors.primaryBg ?? 'bg-slate-800'} ${themeColors.textGrayWhite ?? 'text-gray-100'} py-10 md:py-12 lg:py-16`}>
        ...
        <div className="flex flex-col md:flex-row items-center md:items-start justify-between gap-6 md:gap-10">
          <div className="md:pr-8 text-center md:text-left mb-5 md:mb-0 flex-grow">
            ...
            <div className={`space-y-1 text-sm sm:text-base ${themeColors.textGrayWhite ?? 'text-gray-100'}`}>
                ...
                {email && <p>Email: <ObfuscatedContact value={email} type="email" /></p>}
                {/* Use ObfuscatedContact for phone */}
                {phone && <p>Office telephone: <ObfuscatedContact value={phone} type="phone" /></p>}
                <p>{addressLine1}</p>
                <p>{addressLine2}</p>
          ...
          <div className={`w-40 h-48 md:w-48 md:h-56 lg:w-52 lg:h-60 ${themeColors.backgroundLight ?? 'bg-gray-200'} overflow-hidden rounded-lg flex-shrink-0 border-4 ${themeColors.borderLight ?? 'border-gray-300'} shadow-lg`}>
            <Image src={avatarUrl} alt={nameEn} width={208} height={240} priority unoptimized />
          </div>
        </div>
      </div>
    </div>
