...
function getStatusDisplayName(status: MemberStatus): string {
  switch (status) {
    case MemberStatus.PROFESSOR: return "Professor";
    case MemberStatus.POSTDOC: return "Postdoc";
    case MemberStatus.PHD_STUDENT: return "PhD Student";
    case MemberStatus.MASTER_STUDENT: return "Master Student";
    case MemberStatus.UNDERGRADUATE: return "Undergraduate";
    case MemberStatus.VISITING_SCHOLAR: return "Visiting Scholar";
    case MemberStatus.RESEARCH_STAFF: return "Research Staff";
    case MemberStatus.ALUMNI: return "Alumni";
    case MemberStatus.OTHER: return "Member";
...
export function MemberCard({ member, isEmojiEnabled }: MemberCardProps) {
  ...
  useEffect(() => {
     ...
     return () => {
        try {
         ...
         } catch(e) { console.error("Error during confetti cleanup:", e); }
          finally { confettiRef.current = null; canvasRef.current = null; }
     };
  ...
  const handleMouseEnterConfetti = () => {
    ...
    const emojis = member.favorite_emojis?.split(',').map(e => e.trim()).filter(Boolean) ?? defaultEmojis;
    const finalEmojis = emojis.length > 0 ? emojis : defaultEmojis;
    const config: ExtendedConfettiConfig = { emojis: finalEmojis, emojiSize: 50, confettiNumber: 10 };
  ...
  return (
    <Link href={`/members/${member.id}`} className="block group h-full">
      ...
      >
        {/* 头像容器 (保持不变) */}
        <div className={`w-20 h-20 rounded-full overflow-hidden mb-4 border-2 ${themeColors.borderLight ?? 'border-gray-300'} group-hover:border-${themeColors.primary ?? 'blue-600'} group-hover:scale-105 transition-all duration-300 flex-shrink-0 z-10 shadow-sm`}>
          <Image
        ...
        <div className="flex flex-col items-center justify-center flex-grow z-10 w-full px-1">
          ...
          <span className={`mt-2 text-xs sm:text-sm font-medium ${themeColors.accentColor ?? 'text-purple-700'} bg-purple-50 dark:bg-purple-900/20 px-2 sm:px-3 py-2 sm:py-1 rounded-full inline-block w-fit`}>
            {/* 【修改】仅非教授且年份存在时显示年份 */}
            {member.status !== MemberStatus.PROFESSOR && member.enrollment_year ? `${member.enrollment_year} ` : ''}
            {/* 始终显示状态名称 */}
