  ...
  React.useEffect(() => {
    if (isOpen && educationId !== null) {
      const fetchEducationData = async () => {
        ...
        setApiError(null);
        try {
          const res = await fetch(`/api/members/${memberId}/education/${educationId}`);
          if (!res.ok) throw new Error("Failed to fetch education data");
          const result = await res.json();
          if (result.success && result.data) {
            reset(result.data); // Populate form with existing data
          } else {
            throw new Error(result.error?.message || "Failed to parse education data");
          }
        } catch (err) {
          console.error("Fetch education error:", err);
          setApiError(err instanceof Error ? err.message : "Error loading data");
        } finally {
  ...
  const onSubmit: SubmitHandler<EducationFormData> = async (data) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        throw new Error(result.error?.message || "Failed to save education data.");
      }
    } catch (err) {
      ...
      <div className="bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-2xl border border-gray-700 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex justify-between items-center mb-4 border-b border-gray-700 pb-3">
          <h3 className="text-xl font-semibold text-green-400">
            {educationId !== null ? "Edit Education Record" : "Add Education Record"}
          </h3>
        ...
        {isLoadingData ? (
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                {/* Degree */} 
                <div>
                    <label htmlFor="degree" className="block text-sm font-medium text-gray-300">Degree *</label>
                    <input type="text" id="degree" {...register("degree", { required: "Degree is required" })} 
                           className={`mt-1 block w-full input-style ${errors.degree ? 'border-red-500' : 'border-gray-600'}`} disabled={isSaving} />
                ...
                <div>
                    ...
                    <input type="text" id="school" {...register("school", { required: "School is required" })} 
                           className={`mt-1 block w-full input-style ${errors.school ? 'border-red-500' : 'border-gray-600'}`} disabled={isSaving} />
                    {errors.school && <p className="error-message">{errors.school.message}</p>}
                </div>
                {/* Field */} 
                <div>
                    <label htmlFor="field" className="block text-sm font-medium text-gray-300">Field of Study</label>
                    <input type="text" id="field" {...register("field")} 
                           className={`mt-1 block w-full input-style ${errors.field ? 'border-red-500' : 'border-gray-600'}`} disabled={isSaving} />
                    {errors.field && <p className="error-message">{errors.field.message}</p>}
                </div>
                 {/* Start Year / End Year (inline) */} 
                 <div className="grid grid-cols-2 gap-4">
                     <div>
                         <label htmlFor="start_year" className="block text-sm font-medium text-gray-300">Start Year</label>
                         <input type="number" id="start_year" {...register("start_year", { valueAsNumber: true, min: 1950 })} 
                                className={`mt-1 block w-full input-style ${errors.start_year ? 'border-red-500' : 'border-gray-600'}`} disabled={isSaving} placeholder="YYYY"/>
                         {errors.start_year && <p className="error-message">{errors.start_year.message}</p>}
                     </div>
                     <div>
                         <label htmlFor="end_year" className="block text-sm font-medium text-gray-300">End Year</label>
                         <input type="number" id="end_year" {...register("end_year", { valueAsNumber: true, min: 1950 })} 
                                className={`mt-1 block w-full input-style ${errors.end_year ? 'border-red-500' : 'border-gray-600'}`} disabled={isSaving} placeholder="YYYY (or leave blank)"/>
                          {errors.end_year && <p className="error-message">{errors.end_year.message}</p>}
                     </div>
                 </div>
                 {/* Thesis Title */} 
                 <div>
                     <label htmlFor="thesis_title" className="block text-sm font-medium text-gray-300">Thesis Title</label>
                     <input type="text" id="thesis_title" {...register("thesis_title")} 
                            className={`mt-1 block w-full input-style ${errors.thesis_title ? 'border-red-500' : 'border-gray-600'}`} disabled={isSaving} />
                     {errors.thesis_title && <p className="error-message">{errors.thesis_title.message}</p>}
                 </div>
                 {/* Description */} 
                 <div>
                     <label htmlFor="description" className="block text-sm font-medium text-gray-300">Description</label>
                     <textarea id="description" rows={3} {...register("description")} 
                               className={`mt-1 block w-full input-style resize-y ${errors.description ? 'border-red-500' : 'border-gray-600'}`} disabled={isSaving}></textarea>
                     {errors.description && <p className="error-message">{errors.description.message}</p>}
                 </div>
                {/* Display Order (Optional, maybe hidden or for admin only later) */} 
                <div>
                     <label htmlFor="display_order" className="block text-sm font-medium text-gray-300">Display Order</label>
                     <input type="number" id="display_order" defaultValue={0} {...register("display_order", { valueAsNumber: true })} 
                            className={`mt-1 block w-full input-style ${errors.display_order ? 'border-red-500' : 'border-gray-600'}`} disabled={isSaving} />
                     {errors.display_order && <p className="error-message">{errors.display_order.message}</p>}
                 </div>
