...
export function AddMemberForm({ onSuccess, onCancel }: AddMemberFormProps) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    if (selectedFile) {
      console.log(
        `File selected: ${selectedFile.name}, Type: ${selectedFile.type}, Size: ${selectedFile.size}`
      );
    ...
    console.log(
      "Step 1: Creating member with data (including potential avatar URL):",
      { ...memberData, avatar_url: finalAvatarUrl }
    );
    try {
      ...
      if (!createMemberResponse.ok || !createMemberResult.success) {
        ...
        toast.error(
          `Failed to create member: ${createMemberResult.error?.message || createMemberResult.error || "Unknown error"}`
        );
        setIsLoading(false);
      ...
      if (!setPasswordResponse.ok || !setPasswordResult.success) {
        ...
        toast.error(
          `Member created, but failed to set password: ${setPasswordResult.error || "Unknown error"}. Set manually.`
        );
        ...
        toast.success(
          `Member ${values.username} added and password set successfully!`
        );
      ...
      console.error(
        "Error during member creation/password setting process:",
        error
      );
