...
import { Loader2, Inbox, Edit3, Check, Trash2, ExternalLink, BookOpen, Users, Calendar } from 'lucide-react'; // Added more icons
...
const PublicationManager: React.FC<PublicationManagerProps> = ({ onClose }) => {
  ...
  const handleReject = async (id: string) => {
    ...
    if (!window.confirm(`Are you sure you want to reject and delete this pending publication? This action cannot be undone.`)) {
      return;
    }
      ...
      <div className="space-y-8">
        ...
        <div className={`p-6 rounded-lg border ${themeColors.devBorder} ${themeColors.devCardBg} shadow-md`}>
          <div className="flex justify-between items-center mb-4">
            <h4 className={`text-lg font-semibold ${themeColors.devTitleText}`}>Pending Review ({pendingPublications.length})</h4>
            <button 
                onClick={() => { setIsLoading(true); fetchPendingPublications(); }} 
                disabled={isLoading}
          ...
          {isLoading && pendingPublications.length === 0 && ( // Show loader only if list is empty initially
            <div className="flex justify-center items-center py-6">
              <Loader2 size={24} className={`animate-spin ${themeColors.devDescText}`} />
              <span className={`ml-2 ${themeColors.devDescText}`}>Loading pending items...</span>
            </div>
          )}
          ...
          {!error && pendingPublications.length > 0 && ( // Render list even if loading in background
            <ul className="space-y-4">
              {pendingPublications.map((pub) => (
                <li key={pub.id} className={`p-4 rounded-md border ${themeColors.devBorder} bg-gray-700/30 flex flex-col gap-3`}>
                  {/* Top section: Title and Links */}
                  <div className="flex justify-between items-start gap-2">
                     <h5 className={`font-medium flex-1 ${themeColors.devTitleText}`}>{pub.title || 'No Title'}</h5>
                     <div className="flex items-center gap-3 flex-shrink-0 text-xs">
                       {pub.doi_url && (
                         <a href={`https://doi.org/${pub.doi_url}`} target="_blank" rel="noopener noreferrer" title={pub.doi_url} className={`${themeColors.devDescText} hover:text-indigo-400 transition-colors flex items-center gap-1`}>
                           <ExternalLink size={12} /> DOI
                         </a>
                       )}
                       {pub.pdf_url && (
                         <a href={pub.pdf_url} target="_blank" rel="noopener noreferrer" className={`${themeColors.devDescText} hover:text-indigo-400 transition-colors flex items-center gap-1`}>
                  ...
                  {pub.raw_authors && (
                    <div className={`text-xs flex items-start gap-1.5 ${themeColors.devDescText}`}>
                      ...
                      <span className="flex-1 break-words">{pub.raw_authors}</span>
                    </div>
                  )}
                  {/* Bottom section: Venue, Year, Added Date, Actions */}
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 text-xs">
                    <div className={`flex items-center gap-1 ${themeColors.devDescText}`}> 
                       <Calendar size={12} className="flex-shrink-0" />
                       <span>{pub.venue || 'No Venue'} {pub.year ? `(${pub.year})` : ''}</span>
                       <span className="text-gray-500 ml-1"> | Added: {pub.createdAt.toLocaleDateString()}</span>
                    </div>
                    <div className="flex items-center gap-2 flex-shrink-0 self-end sm:self-center">
                       ...
                       >
                          {actionStates[pub.id] ? <Loader2 size={16} className="animate-spin" /> : <Check size={16} />}
                        </button>
                        <button
                        ...
                        >
                          {actionStates[pub.id] ? <Loader2 size={16} className="animate-spin" /> : <Edit3 size={16} />}
                        </button>
                       <button
                       ...
                       >
                          {actionStates[pub.id] ? <Loader2 size={16} className="animate-spin" /> : <Trash2 size={16} />}
                       </button>
                     </div>
                  </div>
                </li>
        ...
        <div className={`p-6 rounded-lg border ${themeColors.devBorder} ${themeColors.devCardBg} shadow-md`}>
          <h4 className={`text-lg font-semibold mb-4 ${themeColors.devTitleText}`}>Edit Existing Publications</h4>
          <p className={`${themeColors.devDescText}`}>List/Editor for approved publications will appear here.</p>
