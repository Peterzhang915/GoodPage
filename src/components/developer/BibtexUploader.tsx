...
const BibtexUploader: React.FC<BibtexUploaderProps> = ({
  onUploadComplete,
}) => {
  ...
  const handleFileValidation = (file: File | null): boolean => {
    ...
    if (file.name.endsWith(".bib") || file.type === "application/x-bibtex") {
      ...
      setUploadStatus({
        type: "error",
        message: "Invalid file type. Please select or drop a .bib file.",
      });
      return false;
    }
  };
  ...
  const performUpload = async (
    dataToSend: File | Blob,
    isPastedContent: boolean = false,
  ) => {
    const loadingSetter = isPastedContent
      ? setIsProcessingText
      : setIsUploading;
    loadingSetter(true);
    setUploadStatus(null);
    ...
    const fileName = isPastedContent
      ? "pasted_content.bib"
      : (dataToSend as File).name;
  ...
  const handleProcessText = () => {
    if (pastedText.trim()) {
      ...
      setUploadStatus({
        type: "error",
        message: "Please paste BibTeX content first.",
      });
    }
  };
  ...
  return (
    ...
    >
      <h4 className={`text-lg font-semibold mb-4 ${themeColors.devTitleText}`}>
        Import Publications
      </h4>
      ...
      >
        ...
        <div className="flex flex-col items-center justify-center pointer-events-none">
          {" "}
          {/* Make inner content non-interactive for drop */}
          <UploadCloud
            size={32}
            className={`mb-2 ${themeColors.devDescText}`}
          />
          <p className={`${themeColors.devDescText} text-sm mb-1`}>
            {selectedFile
              ? `Selected: ${selectedFile.name}`
              : "Drag & drop a .bib file here, or click to select"}
          </p>
          <p className="text-xs text-gray-500">
            Alternative: Paste content below
          </p>
        </div>
      </div>
      ...
      <div className="mb-4">
        <label
          htmlFor="bibtex-paste-area"
          className={`block text-sm font-medium mb-1 ${themeColors.devDescText}`}
        >
      ...
      <button
        ...
        className={
          `w-full inline-flex items-center justify-center px-4 py-2 border rounded-md shadow-sm text-sm font-medium transition-colors 
                     ${
                       !pastedText.trim() || isUploading || isProcessingText
                         ? `${themeColors.devButtonDisabledBorder} ${themeColors.devDisabledText} ${themeColors.devButtonDisabledBg} cursor-not-allowed`
                         : `border-transparent bg-teal-600 ${themeColors.devButtonText} hover:bg-teal-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-teal-500`
      ...
      {uploadStatus && (
        <div
          className={`mt-4 p-3 rounded-md text-sm flex items-center ${uploadStatus.type === "success" ? "bg-green-900/50 text-green-300" : "bg-red-900/50 text-red-300"}`}
        >
