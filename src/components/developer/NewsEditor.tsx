...
const NewsEditor: React.FC<NewsEditorProps> = ({ onClose }) => {
  ...
  const handleDeleteHistory = async (timestampToDelete: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 404) {
          setHistoryError(
            `History entry not found (timestamp: ${timestampToDelete}). It might have been already deleted.`,
          );
        } else {
        ...
        setStatusMessage(
          `History entry from ${formatDate(timestampToDelete)} deleted.`,
        );
        setMessageType("success");
        setTimeout(() => setStatusMessage(null), 3000);
      }
  ...
  return (
    ...
    <>
      ...
      {statusMessage && (
        <div
          className={`mt-3 text-sm px-3 py-1.5 rounded ${messageType === "success" ? `bg-green-900/80 text-green-100` : messageType === "error" ? `bg-red-900/80 text-red-100` : `bg-blue-900/80 text-blue-100`}`}
        >
          {statusMessage}
        </div>
      )}
      {historyError && (
        <div
          className={`mt-3 text-sm px-3 py-1.5 rounded bg-red-900/80 text-red-100`}
        >
      ...
      <div className="flex items-center justify-end gap-4 mt-4">
        ...
        <button
          ...
          disabled={
            isLoading || isSaving || isLoadingHistory || !!isLoadingDelete
          }
        ...
        >
          ...
          >
              ...
              {isLoadingHistory ? (
                ...
                <ul className="space-y-3">
                  {newsHistory.map((entry) => (
                    <li
                      key={entry.timestamp} // 使用时间戳或特殊值作为 key
                    ...
                    >
                      ...
                      <div className="flex-grow overflow-hidden">
                        <span
                          className={`block text-base font-medium mb-1 truncate ${entry.isLive ? "text-green-300" : entry.isDraft ? "text-yellow-300" : "text-gray-200"}`}
                        >
                          {entry.title || "(Untitled)"}
                          {entry.isLive && (
                            <span className="text-xs font-normal bg-green-800 text-green-200 rounded px-1.5 py-0.5 ml-2 inline-flex items-center">
                              <Radio size={12} className="mr-1" />
                              Live
                            </span>
                          )}
                          {entry.isDraft && (
                            <span className="text-xs font-normal bg-yellow-800 text-yellow-200 rounded px-1.5 py-0.5 ml-2 inline-flex items-center">
                        ...
                        <span
                          className={`block text-xs ${entry.isLive ? "text-gray-300" : entry.isDraft ? "text-gray-300" : "text-gray-400"}`}
                        >
                          {entry.isLive
                        ...
                        <span
                          className={`block text-xs mt-1 ${entry.isLive ? "text-gray-300" : entry.isDraft ? "text-gray-300" : "text-gray-400"}`}
                        >
                          {entry.news.length} lines
                        </span>
                      </div>
            ...
            <div className="mt-4 pt-3 border-t border-gray-600 flex justify-end">
              <button
                onClick={() => setShowHistoryModal(false)}
                className={`px-4 py-2 border rounded-md text-sm font-medium border-gray-600 text-gray-300 bg-gray-700 hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-indigo-500 transition-colors`}
              >
                Close
              </button>
            </div>
