...
export function usePhotoOperations({
  ...
  const updatePhotoOrder = useCallback(
    async (photo: GalleryImage, newOrder: number) => {
      try {
        const albumsView = isAlbumsView(category);
        const currentOrder = albumsView
          ? photo.albums_order
          : photo.display_order;
      ...
      try {
        ...
        onError?.(
          error instanceof Error
            ? error.message
            : "Failed to update photo information"
        );
      }
    },
   ...
   * 批量更新图片顺序
   */
  const batchUpdateOrder = useCallback(
    async (photos: GalleryImage[]) => {
      try {
        const albumsView = isAlbumsView(category);
  ...
  const canMoveDown = useCallback(
    (photo: GalleryImage, totalCount: number) => {
      const albumsView = isAlbumsView(category);
      const currentOrder = albumsView
        ? photo.albums_order
        : photo.display_order;
   ...
   * 获取图片的可见性状态
   */
  const getPhotoVisibility = useCallback(
    (photo: GalleryImage) => {
      const albumsView = isAlbumsView(category);
   ...
   * 获取图片的顺序值
   */
  const getPhotoOrder = useCallback(
    (photo: GalleryImage) => {
      const albumsView = isAlbumsView(category);
