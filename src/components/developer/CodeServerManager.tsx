...
import { ArrowLeft, Server, RefreshCw, Wifi, WifiOff, Loader2, Edit, Trash2, Plus, Save, XCircle } from 'lucide-react';
...
const MOCK_SERVERS_INITIAL: CodeServerInstance[] = [
  ...
  { id: 'cs2', name: 'GPU Server (Experimental)', url: 'http://gpu.example.com:8080' },
...
const CodeServerManager: React.FC<CodeServerManagerProps> = ({ onClose }) => {
  const [servers, setServers] = useState<CodeServerInstance[]>(MOCK_SERVERS_INITIAL);
  ...
  const [editFormData, setEditFormData] = useState<EditFormData>({ name: '', url: '' });
  ...
  const handleDeleteServer = (idToDelete: string) => {
    if (confirm('Are you sure you want to delete this server entry?')) {
        setServers(prevServers => prevServers.filter(server => server.id !== idToDelete));
         // 如果正在编辑的项被删除，则取消编辑状态
  ...
  return (
    <div>
      ...
      {/* 添加服务器表单 */} 
      <form onSubmit={handleAddServer} className="mb-8 p-4 bg-gray-800 border border-gray-700 rounded-lg flex items-end gap-4 relative">
        <div className="flex-grow">
            <label htmlFor="server-name" className="block text-sm font-medium text-gray-300 mb-1">Server Name</label>
            <input 
                type="text"
                id="server-name"
                value={newName}
         ...
         <div className="flex-grow">
            <label htmlFor="server-url" className="block text-sm font-medium text-gray-300 mb-1">Server URL</label>
            <input 
                type="text"
                id="server-url"
                value={newUrl}
        ...
        {/* 添加错误显示位置调整 */} 
        {addError && <p className="text-xs text-red-400 absolute bottom-[-18px] left-4">{addError}</p>}
      </form>

      {/* 服务器列表 */} 
      <div className="space-y-3">
        {servers.length === 0 && (
            <p className="text-center text-gray-500 py-4">No servers configured yet.</p>
        )}
        {servers.map((server) => (
          <div key={server.id} className={`p-4 rounded-lg border flex items-center justify-between gap-4 ${editingServerId === server.id ? 'bg-gray-700 border-indigo-500' : 'bg-gray-800 border-gray-700'}`}>
            {/* 左侧：根据是否编辑显示不同内容 */}
            <div className="flex items-center gap-3 flex-grow overflow-hidden">
                {editingServerId === server.id ? (
                    ...
                    <div className="flex gap-2 flex-grow items-center relative">
                        ...
                        />
                        {/* 编辑错误显示 */} 
                        {editError && <p className="text-xs text-red-400 absolute bottom-[-18px] left-0 w-full truncate" title={editError}>{editError}</p>}
                    </div>
                ) : (
                    // 查看模式：显示文本和链接
                    <div className="overflow-hidden">
                        <p className="text-base font-medium text-gray-100 truncate">{server.name}</p>
                        <a 
                            href={server.url} 
                            target="_blank" 
                            ...
                            title={server.url}
                            onClick={(e) => { if (editingServerId) e.preventDefault(); }} // 编辑时阻止跳转
                        >
                            {server.url}
                        </a>
                    </div>
