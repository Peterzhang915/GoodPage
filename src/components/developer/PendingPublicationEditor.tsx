...
const PendingPublicationEditor: React.FC<PendingPublicationEditorProps> = ({
  ...
  const handleCcfChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const { name, value } = e.target;
    setFormData((prev) =>
      prev ? { ...prev, [name]: value === "N/A" ? null : value } : null,
    );
  };
  ...
  if (error && !formData) {
    // Show error only if data loading failed completely
    return (
      <div
  ...
  if (!formData) {
    ...
    return (
      <div className={`${themeColors.devDescText} p-4`}>
  ...
  const getSuggestedRank = () => {
    ...
    for (const key in ccfRankings) {
      if (venueLower.includes(key.toLowerCase())) {
        console.log(
          `Suggesting rank ${ccfRankings[key]} based on substring match for ${key}`,
        );
        return ccfRankings[key];
      }
    }
  ...
  return (
    <div
      className={`p-6 rounded-lg border ${themeColors.devBorder} ${themeColors.devCardBg} shadow-lg relative`}
    >
      <button
        onClick={onCancel}
      ...
      <h3 className={`text-xl font-semibold mb-6 ${themeColors.devTitleText}`}>
        Edit Publication Details
      </h3>

      {error &&
        !isSaving && ( // Show non-saving errors here
          <div
            className={`mb-4 p-3 rounded-md text-sm flex items-center bg-red-900/50 text-red-300`}
          >
      ...
      >
        ...
        <div>
          ...
          <p
            className={`text-xs p-2 rounded bg-gray-700/50 ${themeColors.devDescText}`}
          >
        ...
        <div>
          ...
          <Select<MemberOption, true>
            ...
            placeholder={
              isLoadingMembers ? "Loading members..." : "Select lab members..."
            }
            className="mt-1 basic-multi-select"
            classNamePrefix="select"
            styles={{
              // Kept previous dark mode styles
              control: (base, state) => ({
                ...base,
                backgroundColor: state.isDisabled
                  ? "#374151"
                  : themeColors.devCardBg.replace("bg-", ""),
                borderColor: themeColors.devBorder.replace("border-", ""),
                color: themeColors.devDescText.replace("text-", ""),
                "&:hover": {
                  borderColor: state.isDisabled
                    ? themeColors.devBorder.replace("border-", "")
                    : "#4b5563",
                },
              }),
              menu: (base) => ({
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            ...
            {suggestedRank &&
              formData.ccf_rank !== suggestedRank &&
              suggestedRank !== "N/A" && (
                <span
                  className="text-xs text-yellow-400 flex items-center gap-1"
                  title={`Based on venue: ${formData.venue}`}
                >
                  <Info size={12} /> Suggested: {suggestedRank}
                </span>
              )}
          </div>
          <p className={`text-xs mt-1 ${themeColors.devDescText}`}>
            Select N/A if not applicable or unknown. Please verify based on the
            official CCF directory.
          </p>
        </div>
