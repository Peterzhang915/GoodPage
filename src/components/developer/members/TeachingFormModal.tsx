...
const teachingSchema = z.object({
  ...
  description_url: z.string().url("Invalid URL format (e.g., https://...)").optional().or(z.literal('')).nullable(), // Allow empty string or valid URL
...
export function TeachingFormModal({
  ...
  const {
    ...
    defaultValues: { // Set defaults based on initialData or empty/undefined
      ...
      <DialogContent className="sm:max-w-[550px] dark:bg-gray-850 dark:border-gray-700">
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="course_title" className="text-right dark:text-gray-300">
            ...
            {errors.course_title && <p className="col-start-2 col-span-3 text-sm text-red-500 dark:text-red-400">{errors.course_title.message}</p>}

            {/* Course Code (Optional) */}
            <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="course_code" className="text-right dark:text-gray-300">Code</Label>
                <Input
                    id="course_code"
            ...
            {/* No error message needed for optional field unless specific validation */}
             ...
             {/* Semester (Optional) */}
             <div className="grid grid-cols-4 items-center gap-4">
                 <Label htmlFor="semester" className="text-right dark:text-gray-300">Semester</Label>
                 <Input
                     id="semester"
                     {...register("semester")}
                     className="col-span-3 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100"
             ...
             {/* University (Optional) */}
             <div className="grid grid-cols-4 items-center gap-4">
                 <Label htmlFor="university" className="text-right dark:text-gray-300">University</Label>
                 <Input
                     id="university"
                     {...register("university")}
                     className="col-span-3 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100"
             ...
             <div className="grid grid-cols-4 items-center gap-4">
                 <Label htmlFor="description_url" className="text-right dark:text-gray-300">Course URL</Label>
                 <Input
                     id="description_url"
                     type="url"
             ...
             {errors.description_url && <p className="col-start-2 col-span-3 text-sm text-red-500 dark:text-red-400">{errors.description_url.message}</p>}
            ...
            <DialogFooter className="mt-4">
                <DialogClose asChild>
                 ...
                 <Button type="submit" disabled={isSubmitting} className="dark:bg-green-600 dark:hover:bg-green-700 dark:text-white">
                     {isSubmitting ? 'Saving...' : (isEditing ? 'Save Changes' : 'Add Teaching')}
                 </Button>
            </DialogFooter>
        </form>
