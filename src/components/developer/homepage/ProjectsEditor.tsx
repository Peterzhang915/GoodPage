...
import {
  ...
  ChevronDown,
  ChevronUp,
} from "lucide-react"; // Added LinkIcon, ChevronDown, ChevronUp
import {
  DndContext,
  closestCenter,
  KeyboardSensor,
...
async function uploadImageApi(file: File): Promise<string> {
  ...
  try {
    ...
    if (!res.ok || !result.success || !result.data?.url) {
      throw new Error(
        result.error?.message || `Upload failed with status ${res.status}`
      );
    }
...
const ProjectsEditor: React.FC = () => {
  ...
  const [newProjectImagePreviewUrl, setNewProjectImagePreviewUrl] = useState<
    string | null
  >(null); // 图片预览 URL 状态
  ...
  // 处理新项目图片选择/拖放
  const handleNewProjectImageSelect = (
  ...
  const handleToggleVisibility = async (
    ...
    const originalItems = projects.map((i) => ({ ...i }));
    setProjects((prev) =>
      prev.map((i) => (i.id === id ? { ...i, is_visible: newVisibility } : i))
    );
    setError(null);
    try {
      await fetchApi(`${apiBaseUrl}/${id}`, {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      ...
      const updates = reordered.map((item: Project, index: number) => ({
        id: item.id,
        display_order: index,
      }));
      ...
      setError(null);
      try {
        await fetchApi(`${apiBaseUrl}/reorder`, {
          // 需要端点 /api/homepage/projects/reorder
          method: "PUT",
          body: JSON.stringify({ items: updates }),
        });
        setProjects(
          reordered.map((item, index) => ({ ...item, display_order: index }))
        );
      } catch (err: any) {
        setError(
          `Failed to save order: ${err.message}`
        ); /* Keep reordered state */
      } finally {
  ...
  return (
    <div className="p-6 h-full flex flex-col">
      ...
      {error && (
        <div
          className={`${themeColors.errorBg} ${themeColors.errorText} p-3 rounded-md mb-4 flex items-center`}
        >
      ...
      <div className="mb-6 border rounded-md ${themeColors.devBorder} ${themeColors.devMutedBg}">
        <div className={`flex items-center justify-between p-4`}>
          ...
          >
            {isAddFormOpen ? (
              <ChevronUp size={20} className={themeColors.devDescText} />
            ) : (
              <ChevronDown size={20} className={themeColors.devDescText} />
            )}
          </button>
        </div>
            ...
            >
              <div className="p-4 pt-2 space-y-3">
                ...
                <div className="space-y-1">
                  <label
                    className={`block text-sm font-medium ${themeColors.devDescText}`}
                  >
                  ...
                  >
                    <div className="space-y-1 text-center">
                      {newProjectImagePreviewUrl ? (
                        ...
                        <svg
                          className={`mx-auto h-12 w-12 ${themeColors.devDescText}`}
                          stroke="currentColor"
                          fill="none"
                          viewBox="0 0 48 48"
                          aria-hidden="true"
                      ...
                      <div className="flex text-sm text-gray-600 justify-center">
                        ...
                        <p className={`pl-1 ${themeColors.devDescText}`}>
                          or drag and drop
                        </p>
                      </div>
                ...
                <button
                  ...
                  disabled={isAdding || isLoading || !newProjectTitle.trim()}
                  title="Add Project"
      ...
      <div className="mb-4 flex space-x-2">
        <button
          ...
          className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${
            projectTypeFilter === ProjectType.MAIN
        ...
        <button
          ...
          className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${
            projectTypeFilter === ProjectType.FORMER
              ? `${themeColors.devButtonBg} ${themeColors.devButtonText}`
              : `${themeColors.devMutedBg} ${themeColors.devDescText} hover:${themeColors.devText}`
          }`}
        >
      ...
      <div className={`flex-1 overflow-y-auto pr-4 -mr-4`}>
        {isLoading ? (
          <div className="flex justify-center items-center h-full">
            <Loader2
              className={`h-8 w-8 ${themeColors.devAccent} animate-spin`}
            />
          </div>
        ) : filteredProjects.length === 0 ? (
          <div className={`${themeColors.devText} text-center py-10`}>
            No {projectTypeFilter === ProjectType.MAIN ? "main" : "former"}{" "}
            projects found.
          </div>
        ) : (
          <DndContext
          ...
          >
            <SortableContext
              items={filteredProjects.map((i) => i.id.toString())}
                ...
                <AnimatePresence initial={false}>
                  {filteredProjects.map((item) => (
                    <SortableProjectItem
                      ...
                      editingDescription={editingItemDescription}
                      editingImageUrl={editingItemImageUrl}
                      editingLinkUrl={editingItemLinkUrl}
                      onEditStart={handleEditItem}
                      onEditChangeTitle={(value) => setEditingItemTitle(value)}
                      onEditChangeDescription={(value) =>
                        setEditingItemDescription(value)
                      }
                      onEditChangeImageUrl={(value) =>
                        setEditingItemImageUrl(value)
                      } // 临时处理程序
                      onEditChangeLinkUrl={(value) =>
                        setEditingItemLinkUrl(value)
                      }
...
interface SortableProjectItemProps {
  ...
  onEditChangeImageUrl: (value: string) => void;
  onEditChangeLinkUrl: (value: string) => void;
  onEditSave: () => void;
  onEditCancel: () => void;
...
function SortableProjectItem({
  ...
  const {
    ...
    transition,
    isDragging,
  } = useSortable({ id: item.id.toString(), disabled: isEditing }); // 编辑此项目时禁用排序
  const style = {
  ...
  const handleImageDrop = useCallback(
    async (event: React.DragEvent<HTMLDivElement>) => {
      ...
      if (file && file.type.startsWith("image/")) {
        // Basic check, more thorough check happens in API
    ...
    >
      ...
      {!isEditing && (
        <div
          {...attributes}
          {...listeners}
          className={`${themeColors.devDescText} mr-3 mt-1 cursor-grab touch-none ${disabled ? "opacity-50 cursor-not-allowed" : ""}`}
      ...
      {isEditing && (
        <div
          className={`${themeColors.devDescText} mr-3 mt-1 opacity-50 cursor-not-allowed`}
        >
          <GripVertical size={18} />
        </div>
      )}
      ...
      >
        {isUploadingImage && (
          <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center z-10">
            <Loader2
              className={`h-6 w-6 ${themeColors.devAccent} animate-spin`}
            />
          </div>
        )}
        {isEditing && !isUploadingImage && (
          <div
            className={`absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-40 flex items-center justify-center text-white text-xs text-center p-1 transition-opacity duration-200 ${item.image_url ? "opacity-0 group-hover:opacity-100" : "opacity-100"}`}
          >
        ...
        {isEditing ? (
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              onChange={(e) => onEditChangeLinkUrl(e.target.value)}
          ...
          <>
            <span
              className={`font-semibold ${item.is_visible ? themeColors.devText : themeColors.devDisabledText} text-base`}
            >
              {item.title}
            </span>
            <p
              className={`${item.is_visible ? themeColors.devDescText : themeColors.devDisabledText} text-sm`}
            >
              {item.description}
            </p>
            {item.link_url && (
        ...
        {isEditing ? (
          <>
            <button
              onClick={onEditSave}
              disabled={
                isSavingEdit || isUploadingImage || !editingTitle.trim()
              }
              title="Save Changes"
