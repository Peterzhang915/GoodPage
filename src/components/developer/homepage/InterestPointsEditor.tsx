...
const InterestPointsEditor: React.FC = () => {
  ...
  const handleDeleteItem = async (id: number) => {
    if (!window.confirm("Are you sure you want to delete this interest point?"))
      return;
    ...
    try {
      ...
      setInterestPoints(originalItems);
    }
  };
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      // 在状态更新前找到原始索引
      const oldIndex = interestPoints.findIndex(
        (i) => i.id.toString() === active.id
      );
      const newIndex = interestPoints.findIndex(
      ...
      // 为项目和索引添加显式类型
      const updates = reordered.map((item: InterestPoint, index: number) => ({
        id: item.id,
        display_order: index,
      }));
      ...
      try {
        await fetchApi(`${apiBaseUrl}/reorder`, {
          // 需要端点 /api/homepage/interest-points/reorder
          method: "PUT",
    ...
    <div className="p-6 h-full flex flex-col">
            ...
            >
              <div className="p-4 pt-2 space-y-3">
                ...
                <input
                  ...
                  placeholder="Enter new interest point title..."
                ...
                <textarea
                  ...
                  placeholder="Enter new interest point description..."
      ...
      <div className={`flex-1 overflow-y-auto pr-4 -mr-4`}>
        {isLoading ? (
          ...
          >
            <SortableContext
              items={interestPoints.map((i) => i.id.toString())}
...
function SortableInterestPointItem({
  item,
  isEditing,
  isSavingEdit,
  editingTitle,
  ...
  onToggleVisibility,
  disabled,
}: SortableInterestPointItemProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  return (
    ...
    >
      ...
      <div
        {...attributes}
        {...listeners}
        className={`${themeColors.devDescText} mr-3 cursor-grab touch-none ${disabled ? "opacity-50 cursor-not-allowed" : ""}`}
      ...
      {/* 操作按钮 */}
      <div className="flex flex-col space-y-1 ml-2 self-start">
        {" "}
        {/* 改为flex-col并调整间距 */}
        {isEditing ? (
          <>
