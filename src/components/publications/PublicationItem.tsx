...
import { BookOpen, Link as LinkIcon, FileText as FileIcon, Calendar, Users, Copy } from 'lucide-react';
        ...
        <div className={`text-sm ${themeColors.textColorSecondary ?? ''} mb-2 pl-7 flex items-center flex-wrap gap-x-1.5 gap-y-1`}>
          ...
          {pub.displayAuthors.map((author, index) => (
             <React.Fragment key={`${itemKey}-author-${author.order}`}>
                ...
                {index < pub.displayAuthors.length - 1 ? <span className="ml-0.5 mr-0.5">,</span> : ''}
            </React.Fragment>
          ))}
        </div>
      )}
      <div className={`text-xs sm:text-sm ${themeColors.textColorTertiary ?? 'text-gray-500'} mb-3 pl-7 flex flex-wrap items-center gap-x-2 gap-y-1`}> {/* Adjusted gap */}
        ...
        {pub.type && pub.type !== 'CONFERENCE' && pub.type !== 'JOURNAL' && (
          <span className={`px-2 py-0.5 rounded-md text-xs font-medium bg-opacity-15 ${themeColors.backgroundDark ?? 'bg-gray-500'} ${themeColors.textColorSecondary ?? 'text-gray-700'}`}>{pub.type}</span>
        )}
      </div>
      {pub.abstract && (
        <details className={`text-sm ${themeColors.textColorSecondary ?? ''} mt-3 group pl-7`}>
          <summary className={`cursor-pointer ${themeColors.linkColor ?? 'text-blue-600'} hover:underline font-medium list-none group-open:mb-2 text-xs sm:text-sm`}>Show Abstract</summary>
          <p className={`italic ${themeColors.textColorTertiary ?? 'text-gray-600'} border-l-2 ${themeColors.borderLight ?? 'border-gray-300'} pl-3 text-xs sm:text-sm leading-relaxed`}>{pub.abstract}</p>
        </details>
      )}
      {pub.keywords && (
        <div className={`mt-3 text-xs sm:text-sm ${themeColors.textColorTertiary ?? 'text-gray-500'} pl-7`}>
          <span className="font-semibold mr-1">Keywords:</span> {pub.keywords}
        </div>
      )}
      {(pub.dblp_url || pdfHref || pub.slides_url || pub.video_url || pub.code_repository_url || pub.project_page_url) && (
